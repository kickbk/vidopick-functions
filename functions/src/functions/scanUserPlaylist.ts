...
async function sendModerationEmail(playlistData: {
  ...
  if (!EMAIL_ACCOUNT || !EMAIL_PASS) return;
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (existingSnap.exists) {
        console.log(`[scanUserPlaylist] found in scannedPlaylists (approved=${existingSnap.data()?.isApproved})`);
        ...
        res.status(200).json({ source: existingSnap.data()?.isApproved ? "approved" : "scanned", ...existingSnap.data() });
        return;
      }
      ...
      if (error.message === "PLAYLIST_NOT_FOUND") {
        res.status(404).json({ error: "Playlist not found on YouTube. It may be private or deleted." });
      } else {
